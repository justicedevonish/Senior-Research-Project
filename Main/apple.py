...
def detect_fractal_patterns(stock_data, tolerance=0.02):
    ...
    avg_price = stock_data['Close AAPL'].mean()  # Average price for tolerance calculation

    for i in range(2, len(stock_data) - 2):
        # Basic Bullish Fractal (Bottom)
        if (stock_data['Low AAPL'][i] < stock_data['Low AAPL'][i-1] and 
            stock_data['Low AAPL'][i] < stock_data['Low AAPL'][i-2] and
            stock_data['Low AAPL'][i] < stock_data['Low AAPL'][i+1] and 
            stock_data['Low AAPL'][i] < stock_data['Low AAPL'][i+2]):
            stock_data.at[stock_data.index[i], 'Pattern'] = "Bullish Fractal"

        # Basic Bearish Fractal (Top)
        elif (stock_data['High AAPL'][i] > stock_data['High AAPL'][i-1] and 
              stock_data['High AAPL'][i] > stock_data['High AAPL'][i-2] and
              stock_data['High AAPL'][i] > stock_data['High AAPL'][i+1] and 
              stock_data['High AAPL'][i] > stock_data['High AAPL'][i+2]):
            stock_data.at[stock_data.index[i], 'Pattern'] = "Bearish Fractal"
        
        # Double Bottom (Bullish Reversal)
        elif (i > 2 and abs(stock_data['Low AAPL'][i] - stock_data['Low AAPL'][i-2]) <= tolerance * avg_price):
            stock_data.at[stock_data.index[i], 'Pattern'] = "Double Bottom (Bullish)"
        
        # Double Top (Bearish Reversal)
        elif (i > 2 and abs(stock_data['High AAPL'][i] - stock_data['High AAPL'][i-2]) <= tolerance * avg_price):
...
def calculate_predictive_accuracy(stock_data, prediction_window=5):
    ...
    for i in range(len(stock_data) - prediction_window):
        ...
        if pattern in pattern_counts:
            ...
            future_prices = stock_data['Close AAPL'].iloc[i+1:i+1+prediction_window]

            # Define "correct" prediction based on pattern type
            if pattern == "Bullish Fractal" and future_prices.max() > stock_data['Close AAPL'].iloc[i]:
                correct_predictions[pattern] += 1
            elif pattern == "Bearish Fractal" and future_prices.min() < stock_data['Close AAPL'].iloc[i]:
                correct_predictions[pattern] += 1
            elif pattern == "Double Bottom (Bullish)" and future_prices.max() > stock_data['Close AAPL'].iloc[i]:
                correct_predictions[pattern] += 1
            elif pattern == "Double Top (Bearish)" and future_prices.min() < stock_data['Close AAPL'].iloc[i]:
                correct_predictions[pattern] += 1
...
def plot_fractal_patterns(stock_data, ticker):
    ...
    plt.plot(stock_data['Close AAPL'], label='Close Price')
...
if __name__ == "__main__":
    ...
    if 'High AAPL' in stock_data.columns and 'Low AAPL' in stock_data.columns:
